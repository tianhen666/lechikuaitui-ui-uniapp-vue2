  ...
  >
    <view :class="'zero-lazy-item-' + elIndex" style="position: relative;">
      <view class="loading-container" v-if="(!isShow || loading) && !isError">
        <image class="loading" :src="loadingImg" mode="aspectFill"></image>
      </view>
      <image
        :style="{ borderRadius: borderRadius, height: imgHeight }"
        v-if="!isError && image"
        class="zero-lazy-item"
        :src="isShow ? image : load"
        :mode="imgMode"
        @load="imgLoaded"
        @error="loadError"
        @tap.stop="clickImg"
      ...
      <image
        :style="{
          ...
          'background-size': avator ? 'cover' : ''
        }"
        class="zero-lazy-item error"
        v-else
        :src="load"
        :mode="imgMode"
        @load="errorImgLoaded"
